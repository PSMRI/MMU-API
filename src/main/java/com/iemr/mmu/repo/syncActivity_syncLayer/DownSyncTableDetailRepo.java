...
@Repository
public interface DownSyncTableDetailRepo extends CrudRepository<DownSyncTableDetail, Integer> {

	@Query(" SELECT d FROM DownSyncTableDetail d WHERE d.isActive = true ORDER BY d.syncOrder, d.downSyncTableDetailID ")
	ArrayList<DownSyncTableDetail> getActiveDownSyncTables();

	@Query(" SELECT d FROM DownSyncTableDetail d WHERE d.isActive = true AND lower(d.tableName) = lower(:tableName) ")
	ArrayList<DownSyncTableDetail> getActiveDownSyncTableByName(@Param("tableName") String tableName);
}
