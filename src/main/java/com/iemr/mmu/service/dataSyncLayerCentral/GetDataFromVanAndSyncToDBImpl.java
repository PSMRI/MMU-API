...
public class GetDataFromVanAndSyncToDBImpl implements GetDataFromVanAndSyncToDB {
    ...
    public String syncDataToServer(String requestOBJ, String Authorization) throws Exception {
        ...
        if ("m_beneficiaryregidmapping".equalsIgnoreCase(syncTableName)) {
            ...
            // In syncDataToServer - instead of one huge response, break it down
            Map<String, Object> responseMap = new HashMap<>();
            responseMap.put("statusCode", 200);
            responseMap.put("message", "Data sync completed");

            // Group records by status to reduce data
            Map<String, List<String>> groupedResults = new HashMap<>();
            groupedResults.put("success", new ArrayList<>());
            groupedResults.put("failed", new ArrayList<>());

            Map<String, String> failureReasonMap = new HashMap<>();

            for (SyncResult sr : syncResults) {
                if (sr.isSuccess()) {
                    groupedResults.get("success").add(sr.getVanSerialNo());
                } else {
                    groupedResults.get("failed").add(sr.getVanSerialNo());
                    if (sr.getReason() != null) {
                        failureReasonMap.put(sr.getVanSerialNo(), sr.getReason());
                    }
                }
            }

            Map<String, Object> dataMap = new HashMap<>();
            dataMap.put("successVanSerialNos", groupedResults.get("success"));
            dataMap.put("failedVanSerialNos", groupedResults.get("failed"));
            dataMap.put("failureReasons", failureReasonMap);
    ...
    // private boolean performGenericTableSync(SyncUploadDataDigester
    // syncUploadDataDigester,
    // List<SyncResult> syncResults) {
    ...
    // Map<Integer, Integer> insertIndexMap = new HashMap<>(); // syncResults index
    // -> insert list index
    // Map<Integer, Integer> updateIndexMap = new HashMap<>(); // syncResults index
    // -> update list index

    // boolean overallSuccess = true;

    // if (dataToBesync == null || dataToBesync.isEmpty()) {
    // logger.info("No data to sync for table: {}",
    // syncUploadDataDigester.getTableName());
    // return true;
    // }
    ...
    // syncUploadDataDigester.getVanAutoIncColumnName();
    // String schemaName = syncUploadDataDigester.getSchemaName();
    // Integer facilityIDFromDigester = syncUploadDataDigester.getFacilityID();
    // String serverColumns = syncUploadDataDigester.getServerColumns();

    // int vanSerialIndex =
    // Arrays.asList(serverColumns.split(",")).indexOf(vanAutoIncColumnName);
    // List<String> serverColumnsList = Arrays.asList(serverColumns.split(","));
    ...
    // if (cleanRecord.containsKey("TransferToFacilityID")
    // && cleanRecord.get("TransferToFacilityID") instanceof Number) {
    // Number transferToFacilityID = (Number)
    // cleanRecord.get("TransferToFacilityID");
    // if (transferToFacilityID.intValue() == facilityIDFromDigester) {
    // cleanRecord.put("Processed", "P");
    // }
    // }
    ...
    // }
    // case "t_itemstockentry": {
    // if (cleanRecord.containsKey("FacilityID") && cleanRecord.get("FacilityID")
    // instanceof Number) {
    // Number mapFacilityID = (Number) cleanRecord.get("FacilityID");
    // if (mapFacilityID.intValue() == facilityIDFromDigester) {
    // cleanRecord.put("Processed", "P");
    ...
    // if (cleanRecord.containsKey("SyncFacilityID") &&
    // cleanRecord.get("SyncFacilityID") instanceof Number) {
    // syncFacilityID = ((Number) cleanRecord.get("SyncFacilityID")).intValue();
    // }
    ...
    // try {
    // recordCheck = dataSyncRepositoryCentral.checkRecordIsAlreadyPresentOrNot(
    // schemaName, syncTableName, vanSerialNo, vanID, vanAutoIncColumnName,
    // syncFacilityID);
    // logger.info("Record check result: {}", recordCheck);
    // } catch (Exception e) {
    // logger.error("Error checking record existence for table {}: VanSerialNo={},
    // VanID={}. Error: {}",
    // syncTableName, vanSerialNo, vanID, e.getMessage(), e);
    ...
    // int currentSyncResultIndex = syncResults.size();
    // syncResults.add(new SyncResult(schemaName, syncTableName, vanSerialNo,
    // syncUploadDataDigester.getSyncedBy(), false, "Pending")); // Initially set as
    // success
    ...
    // if (Arrays.asList("t_patientissue", "t_physicalstockentry",
    // "t_stockadjustment", "t_saitemmapping",
    // "t_stocktransfer", "t_patientreturn", "t_facilityconsumption", "t_indent",
    // "t_indentorder", "t_indentissue", "t_itemstockentry", "t_itemstockexit")
    // .contains(syncTableName.toLowerCase()) &&
    // cleanRecord.containsKey("SyncFacilityID")) {
    // updateParams.add(String.valueOf(cleanRecord.get("SyncFacilityID")));
    // } else {
    // updateParams.add(String.valueOf(vanID));
    // }
    ...
    // // Process INSERT operations
    // if (!syncDataListInsert.isEmpty()) {
    // String queryInsert = getQueryToInsertDataToServerDB(schemaName,
    // syncTableName, serverColumns);

    // try {
    // int[] insertResults =
    // dataSyncRepositoryCentral.syncDataToCentralDB(schemaName, syncTableName,
    // serverColumns, queryInsert, syncDataListInsert);
    ...
    // boolean success = insertListIndex < insertResults.length &&
    // insertResults[insertListIndex] > 0;

    // if (!success) {
    // String failedVanSerialNo =
    // getVanSerialNo(syncDataListInsert.get(insertListIndex),
    // vanSerialIndex,
    // syncResults.get(syncResultIndex));
    // String conciseReason = "Insert failed (code: " +
    // (insertListIndex < insertResults.length ? insertResults[insertListIndex] :
    // "unknown") +
    // ")";
    // syncResults.set(syncResultIndex, new SyncResult(schemaName, syncTableName,
    // failedVanSerialNo,
    ...
    // String vanSerialNo = getVanSerialNo(syncDataListInsert.get(insertListIndex),
    // vanSerialIndex,
    // syncResults.get(syncResultIndex));
    // int vanIDIndex = serverColumnsList.indexOf("VanID");
    // String vanID = vanIDIndex >= 0 && vanIDIndex <
    // syncDataListInsert.get(insertListIndex).length
    ...
    // int recordExists =
    // dataSyncRepositoryCentral.checkRecordIsAlreadyPresentOrNot(
    ...
    // if (!syncDataListUpdate.isEmpty()) {
    // String queryUpdate = getQueryToUpdateDataToServerDB(schemaName,
    // serverColumns, syncTableName);
    ...
    // boolean success = updateListIndex < updateResults.length &&
    // updateResults[updateListIndex] > 0;

    // if (!success) {
    // String failedVanSerialNo =
    // getVanSerialNo(syncDataListUpdate.get(updateListIndex),
    // vanSerialIndex,
    // syncResults.get(syncResultIndex));
    // String conciseReason = "Update failed (code: " +
    // (updateListIndex < updateResults.length ? updateResults[updateListIndex] :
    // "unknown") +
    // ")";
    // syncResults.set(syncResultIndex, new SyncResult(schemaName, syncTableName,
    // failedVanSerialNo,
    ...
    // String vanSerialNo = getVanSerialNo(syncDataListUpdate.get(updateListIndex),
    // vanSerialIndex,
    // syncResults.get(syncResultIndex));
    // int vanIDIndex = serverColumnsList.indexOf("VanID");
    // String vanID = vanIDIndex >= 0 && vanIDIndex <
    // syncDataListUpdate.get(updateListIndex).length
